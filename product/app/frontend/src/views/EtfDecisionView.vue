 ...
 * 职责：
 * - 调用后端 ETF 决策接口。
 * - 展示当前规则结论、三项指标卡片、数据时间和接口说明入口。
 *
 * 边界：
 * - 页面不自行拉取外部市场数据，不重新计算阈值规则。
 */
import { computed, onMounted, ref } from 'vue'
import MetricCard from '../components/MetricCard.vue'
import { apiUrl, docsUrl } from '../config/project.js'
...
async function loadDecision(force = false) {
  // force=true 会要求后端绕过缓存，适合用户手动刷新验证。
  error.value = ''
  if (force) refreshing.value = true
  else loading.value = true
  try {
    const response = await fetch(`${apiUrl('/modules/etf-buy-decision')}${force ? '?refresh=true' : ''}`)
  ...
  <section class="decision-page content-width">
    ...
    <template v-else-if="data">
      ...
      <aside class="method-note">
        ...
        <a :href="docsUrl()" target="_blank" rel="noreferrer">查看 API 口径 ↗</a>
      </aside>
    </template>
  </section>
</template>
