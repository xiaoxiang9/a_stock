...
其中 `Tushare`、`mx-finance-data`、`mx-finance-search` 的 skill 副本已固化到仓库 `skills/released/`，数据层在需要调用 skill 脚本时优先读取仓库内版本，避免依赖服务器本机的全局安装态；旧名 `mx-data`、`mx-search` 仅作为迁移期兼容别名。
