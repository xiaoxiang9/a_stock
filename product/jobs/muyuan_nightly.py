...
- 渲染 Markdown 报告、HTML 邮件，并支持通过 msmtp 发送。
...
from product.core.model_service import ModelService, build_codex_exec_command as _build_codex_exec_command
from product.core.project_config import ModelRuntimeConfig, load_project_config
...
def build_email_subject(report_date: str) -> str:
    """生成邮件主题，保持日报邮件可按日期检索。"""
    return f"牧原股份 {report_date} 21:00 日复盘"
...
def render_report_markdown(
    ...
    return f"""# {report_date} 牧原股份 21:00 日复盘
...
def render_launchd_plist(
    ...
    script_path = f"{project_root}/product/jobs/muyuan_nightly.py"
    log_path = f"{project_root}/product/jobs/muyuan_nightly.log"
    xml_items = "\n".join(
        [
...
def load_env_from_bash_profile() -> None:
    ...
    主要用于本地定时任务场景，避免 launchd 启动时拿不到 token。
    """
...
def send_email(recipient: str, subject: str, text_body: str, html_body: str) -> None:
    ...
    if result.returncode != 0:
        raise RuntimeError(result.stderr.strip() or "msmtp send failed")
...
def main() -> int:
    ...
    output_path, content, report_context = generate_report(report_date=report_date, force=args.force)
    ...
    if args.send_email:
        ...
        send_email(args.recipient, subject, content, html_body)
