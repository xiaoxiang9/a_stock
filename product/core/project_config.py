...
职责：
- 从 `product/config/project.toml` 读取运行配置。
...
DEFAULT_CONFIG_PATH = ROOT / "product" / "config" / "project.toml"
...
def _parse_toml_fallback(raw_text: str) -> dict[str, Any]:
    ...
    注意：这不是通用 TOML 解析器，只服务于 product/config/project.toml
...
def _load_project_config_uncached(config_path: Path) -> ProjectConfig:
    ...
    launchd = LaunchdConfig(
        ...
        hour=int(launchd_data.get("hour", 21)),
        minute=int(launchd_data.get("minute", 0)),
    )
    tushare = TushareConfig(
        ...
        start_date=_clean_optional_text(tushare_data.get("start_date")) or "20260401",
        end_date=_clean_optional_text(tushare_data.get("end_date")) or "20261231",
    )
    report = ReportConfig(
